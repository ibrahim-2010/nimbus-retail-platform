...
- **Issues Report** – 19 production issues documented with root cause, fix, and lesson learned
...
**19 production issues resolved** across multiple deployment cycles. This is not a tutorial – these are real failures with real debugging and real fixes. Full report: [`docs/ISSUES-REPORT.md`](docs/ISSUES-REPORT.md).
...
| 17 | Prometheus DNS not resolving after redeploy | Route 53 alias pointed to dead ALB from previous deployment; ExternalDNS wouldn't update it because the TXT ownership record was missing | Updated Route 53 alias to live ALB + added two TXT ownership records | ExternalDNS only manages records it owns – without the TXT record it treats a stale alias as externally managed and leaves it alone |
| 18 | Tempo datasource `i/o timeout` in Grafana | Datasource URL used port 3100 (Loki's port); Tempo exposes its HTTP query API on port 3200 | Corrected URL to `http://tempo:3200` in `helm-monitoring.tf` | Different observability tools use different default ports – verify per service, not per stack |
| 19 | Loki "Unable to connect" in Grafana | Grafana 13 changed its Loki health check to a newer LogQL syntax that Loki 2.9.3 (shipped by deprecated `loki-stack` chart) cannot parse | Replaced `loki-stack` with new `loki` 3.x chart (singleBinary mode) + separate `promtail` chart | Deprecated charts cap at older versions; Grafana 13 compatibility requires the current `loki` chart – Loki itself was healthy, only the health check query failed |
...
| [`docs/ISSUES-REPORT.md`](docs/ISSUES-REPORT.md) | 19 production issues – root cause, fix, and lesson for each |
